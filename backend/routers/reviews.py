    ...
    async with pool.acquire() as conn:
        try:
            row = await conn.fetchrow(
                ...
                VALUES ($1, $2, $3, $4)
                RETURNING id, client_id, service_id, rating, comment
                """,
                review.client_id, review.service_id, review.rating, review.comment
            )
...
async def get_reviews(request: Request):
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT id, client_id, service_id, rating, comment FROM reviews ORDER BY id DESC"
        )
...
async def get_review(review_id: int, request: Request):
    ...
    async with pool.acquire() as conn:
        review = await conn.fetchrow(
            "SELECT id, client_id, service_id, rating, comment FROM reviews WHERE id = $1",
            review_id
        )
        if not review:
...
async def update_review(review_id: int, review: ReviewModel, request: Request):
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(
            """
            UPDATE reviews
            SET client_id=$1, service_id=$2, rating=$3, comment=$4
            WHERE id=$5
            """,
            review.client_id, review.service_id, review.rating, review.comment, review_id
        )
